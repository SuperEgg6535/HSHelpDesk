...
function filterTickets(subject) {
    ...
    tickets.forEach(ticket => {
        // Show all subjects and check if status is 'On hold'
        if (ticket.status === 'On hold' && (subject === 'All' || ticket.subject === subject)) {
...
document.getElementById('status-filter').addEventListener('change', function () {
    // Only allow "On hold" to be shown
    filtersTickets('On hold');
});
...
function filtersTickets(status) {
    ...
    tickets.forEach(ticket => {
        // Show only tickets that are On hold
...
function addTicketToTable(ticket) {
    ...
    <td><a href="#" onclick="loadTicketDetails('${ticket.id}')">${ticket.requester}</a></td>
        ...
        <td>${ticket.creationDate ? formatDate(new Date(ticket.creationDate)) : 'N/A'}</td> <!-- Display Closure Date -->
    `;
